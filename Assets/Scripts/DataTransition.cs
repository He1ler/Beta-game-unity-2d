...
public static class DataTransition
{
 ...
 // }
  ...
  {
      BinaryFormatter formatter = new BinaryFormatter();
      string path = Application.persistentDataPath + "/" + hero[i].HeroObject.name + ".hero";
      FileStream stream = new FileStream(path, FileMode.OpenOrCreate);
  ...
  public static HeroData HeroFromFile(string HeroName)//load Hero from file into the sting variable
  {
   ...
   {
       string path = Application.persistentDataPath + "/" + enemy[i].EnemyObject.name + ".enemy";
       BinaryFormatter formatter = new BinaryFormatter();
       FileStream stream = new FileStream(path, FileMode.OpenOrCreate);
 ...
 //      stream.Close();
 //  }
 // public static void SaveGame(int i1, int i2, int i3, int i4, int Waves, int EA)
 // {
 //     BinaryFormatter formatter = new BinaryFormatter();
 //     string path = Application.persistentDataPath + "/Name.map";
 //     FileStream stream = new FileStream(path, FileMode.OpenOrCreate);
 //     GameData GameData = new GameData(i1, i2, i3, i4, Waves, EA);
 //     formatter.Serialize(stream, GameData);
 //     stream.Close();
 // }
}
